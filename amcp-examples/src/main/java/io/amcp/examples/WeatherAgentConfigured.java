...
/**
 * Weather Agent with Real Data Integration - v1.6 Quarkus (Configured)
 * 
 * Fetches real weather data from OpenWeatherMap API using configured API key.
 * Falls back to simulated data if API is unavailable.
 * 
 * API Key: Pre-configured (3bd965f39881ba0f116ee0810fdfd058)
 */
@ApplicationScoped
public class WeatherAgentConfigured extends AbstractMobileAgent {
    
    private static final ObjectMapper mapper = new ObjectMapper();
    private static final Logger logger = LoggerFactory.getLogger(WeatherAgentConfigured.class);
    private static final HttpClient httpClient = HttpClient.newHttpClient();
    
    // OpenWeatherMap API configuration - CONFIGURED WITH REAL KEY
    ...
    public void onActivate() {
        ...
        logMessage("🌤️  Weather Agent (Configured) activated - Real API key configured ✅");
    }
    ...
    private void handleWeatherRequest(Event event) {
        try {
            ...
            logMessage("📍 Weather request for: " + city + " (Real Data Mode)");
            ...
            if (weather == null) {
                ...
                logMessage("⚠️  API unavailable - Using fallback simulated data for: " + city);
            }
            
            Map<String, Object> response = new HashMap<>(weather);
            response.put("timestamp", System.currentTimeMillis());
            response.put("source", "weather-agent-configured");
            response.put("dataSource", weather.containsKey("real") ? "openweathermap (real)" : "simulated");
            
            publishEvent("weather.response", response);
            logMessage("✅ Weather response sent for: " + city + " (" + (weather.containsKey("real") ? "Real" : "Simulated") + ")");
    ...
    private Map<String, Object> fetchRealWeatherData(String city) {
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                logger.info("✅ Real weather data fetched for {} from OpenWeatherMap", city);
                return weather;
            } else {
                logger.warn("⚠️  OpenWeatherMap API returned status: {} - falling back to simulated data", response.statusCode());
                return null;
            }
        } catch (Exception e) {
            logger.warn("⚠️  Failed to fetch real weather data: {} - falling back to simulated data", e.getMessage());
            return null;
        }
    }
    ...
    private void handleForecastRequest(Event event) {
        try {
            ...
            logMessage("📊 Forecast request for: " + city + " (Real Data Mode)");
            ...
            logMessage("✅ Forecast response sent for: " + city + " (" + (weather.containsKey("real") ? "Real" : "Simulated") + ")");
    ...
    private void handleStatusRequest(Event event) {
        try {
            ...
            status.put("agent", "WeatherAgentConfigured");
            status.put("version", "1.6.0");
            status.put("status", "active");
            status.put("timestamp", System.currentTimeMillis());
            status.put("apiKey", "✅ Configured");
            status.put("apiProvider", "OpenWeatherMap");
    ...
    public void onDeactivate() {
        logMessage("👋 Weather Agent (Configured) shutting down - API Key: ✅ Configured");
        super.onDeactivate();
    }
}
