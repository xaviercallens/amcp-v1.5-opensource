...
# AMCP v1.6 - Phase 1 & Phase 2 Testing Script
# Tests MicroProfile Health & Metrics + A2A Gateway

set -e

echo "=================================================="
echo "AMCP v1.6 - Phase 1 & Phase 2 Testing"
echo "=================================================="
echo ""
...
check_service "$HEALTH_LIVE_URL" || exit 1
...
if echo "$RESPONSE" | grep -q "broker_type"; then
    test_result "Liveness check includes broker_type" 0
else
    test_result "Liveness check includes broker_type" 1
fi

if echo "$RESPONSE" | grep -q "instance_id"; then
    test_result "Liveness check includes instance_id" 0
else
    test_result "Liveness check includes instance_id" 1
fi
